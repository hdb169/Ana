...
out_dir = os.path.join('/u/group/c-csv/hdbhatt/test_april2020_branch/hcswif/outfiles/', getpass.getuser() , 'hcswif/output')
...
def getCommandJobs(parsed_args, wf_name):
   ...
   # print(job['name'])
   # print (wf_name)
    #if parsed_args.spec==None:
    #    raise RuntimeError("Must specify spectrometer")
   # spec = parsed-args.shell[0]
...
def addCommonJobInfo(workflow, parsed_args):
    ...
    if parsed_args.project==None:
        ...
        while project_prompt.lower() not in ['y', 'n', 'yes', 'no']:
           ...
           # project_prompt = input('Should I use project=c-sidis18? (y/n): ')
    ...
    for n in range(0, len(workflow['jobs'])):
        ...
        # TODO: Allow user to specify all of these parameters:centos77#---------------->changed
        job['os'] = 'centos77'
        job['track'] = 'analysis'
