...
/u/group/c-csv/hdbhatt/my_analysis/efficiency/cal/hcer_efftry.C
...
#define NRUNS 205
void hcer_efftry(){//has hgcer cut
 
  // ofstream txtfile("txtfile-aeroeff-aps/hcer_efficiency_aps.txt",ios::app);
  ofstream txtfile("txtfile_cal_oct15/curr_fadc_hcer_eff.txt",ios::app);
  ...
  for(Int_t r = 0; r<NRUNS;r++){
    ...
    //   Int_t runs[NRUNS]     ={5367, 5368, 5369, 5370, 5371, 5372, 5373, 5374, 5375, 5376, 5377,     6429,6430,6431,6432,6433,   6459,6460,6461,6462,6463,6464   };  
    
  Int_t runs[NRUNS]  = { 7593, 7594, 7595, 7596, 7597, 7599, 7600, 7601, 7602, 7603, 7604, 7605, 7606, 7607, 7608, 7609, 7610, 7611, 7612, 7613, 7614, 7615, 7616, 7617, 7618, 7619, 7620, 7621, 7622, 7623, 7625, 7626, 7627, 7628, 7629, 7630, 7631, 7632, 7633, 7634, 7635, 7636, 7637, 7638, 7639, 7640, 7641, 7642, 7643, 7644, 7645, 7646, 7647, 7648, 7649, 7650, 7651, 7652, 7654, 7655, 7657, 7658, 7659, 7660, 7661, 7662, 7663, 7666, 7667, 7668, 7669, 7670, 7671, 7672, 7673, 7674, 7675, 7676, 7677, 7678, 7679, 7680, 7681, 7682, 7684, 7685, 7686, 7687, 7688, 7689, 7690, 7692, 7694, 7695, 7697, 7698, 7699, 7702, 7703, 7706, 7707, 7708, 7709, 7710, 7711, 7712, 7713, 7714, 7715, 7716, 7717, 7718, 7719, 7720, 7721, 7722, 7724, 7725, 7727, 7728, 7735, 7736, 7737, 7738, 7739, 7740, 7747, 7748, 7749, 7750, 7751, 7752, 7757, 7758, 7759, 7760, 7762, 7768, 7769, 7770, 7771, 7772, 7773, 7774, 7775, 7776, 7777, 7783, 7784, 7785, 7786, 7787, 7790, 7791, 7792, 7793, 7794, 7796, 7797, 7798, 7799, 7800, 7801, 7802, 7803, 7804, 7805, 7806, 7807, 7808, 7809, 7811, 7812, 7813, 7814, 7815, 7816, 7817, 7819, 7820 };
...
#include "scaler_analysis_boil_fadc.h"
	...
	/////////////////////////////////////////////////actual eff is for aero , byr here diff nees, butsl all =mames---------================
	//note this is for aero, not aero
       	if(runs[r]<6000){
	  base_cut_aero = hdelta> -8 && hdelta < 8 && hhodstarttime ==1 && hdipole==1 &&paero >6  && pdelta >-10 && pdelta < 20 &&  phodstarttime == 1 && pdipole ==1 && eventtype > 3 && mod4_pioncut  && xptaryptar_cut && pcaletottrack < 0.5 && hcaletottrack > 1.2 ; 
	}
	if(runs[r]>6000&&runs[r]<7593){
	  base_cut_aero = hdelta> -8 && hdelta < 8 && hhodstarttime ==1 && hdipole==1 && paero >8  && pdelta >-10 && pdelta < 20 &&  phodstarttime == 1 && pdipole ==1 && eventtype > 3 && mod4_pioncut  && xptaryptar_cut && pcaletottrack < 0.65 && hcaletottrack > 0.8 ; 
	}
	if(runs[r]>7593){
	  base_cut_aero = hdelta> -8 && hdelta < 8 && hhodstarttime ==1 && hdipole==1 && paero >8   && pdelta >-10 && pdelta < 20 &&  phodstarttime == 1 && pdipole ==1 && eventtype > 3 && mod4_pioncut  && xptaryptar_cut && pcaletottrack < 0.65 && hcaletottrack > 0.8 &&  ngcer < ngc; 
	}
	...
	if(runs[r] < 6000){
	  paero_did_cut_acc = paero_should_cut_acc && hcernpe > 2;
	  paero_did_cut_coin = paero_should_cut_coin && hcernpe > 2;
	}
    ...
    txtfile<<setprecision(5)<<fixed<<runs[r]<<"\t"<<boil_corr<<"\t"<<fadc_dtime_corr<<"\t"<<aero_eff<<"\t"<<aero_eff_err<<endl;//aero means HCER in FACT
    //cout<<setprecision(5)<<fixed<<runs[r]<<"\t"<< aero_eff<<"\t"<<aero_eff_err<<endl;
