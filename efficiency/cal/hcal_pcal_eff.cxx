#define NRUNS 22
void hcal_pcaleff(){//has hgcer cut
  ofstream txtfile1("txtfile_cal_effSept31/hcal_pcal_eff.txt",ios::app);
  txtfile1<<setprecision(5)<<"runs"<<"\t"<<"hcal"<<"\t"<<"hErr"<<"\t"<< "pcal"<<"\t"<<"pErr"<<endl;

  ofstream txtfile2("txtfile_cal_effSept31/hcal_coin_acccoin_clean_integral.txt",ios::app);
  txtfile2<<setprecision(5)<<"runs"<<"\t"<<"hdidcoin"<<"\t"<<"hdidacc"<<"\t"<< "hdidcl"<<"\t"<<"hshouldcoin"<<"\t"<<"hshouldacc"<<"\t"<< "hshouldcl"<<"\t"<<"heff"<<"\t"<<"hErr"<<endl;

  ofstream txtfile3("txtfile_cal_effSept31/pcal_coin_acccoin_clean_integral.txt",ios::app);
  txtfile3<<setprecision(5)<<"runs"<<"\t"<<"pdidcoin"<<"\t"<<"pdidacc"<<"\t"<< "pdidcl"<<"\t"<<"pshouldcoin"<<"\t"<<"pshouldacc"<<"\t"<< "pshouldcl"<<"\t"<<"peff"<<"\t"<<"pErr"<<endl;
...
#include "double_good.h"
    ...
    Double_t pcaletotmax_peter = 0.5;
    Double_t hcaletotmin_peter = 1.2;


    Double_t aeromin = 6;////5
    Double_t hgcermin = 5;  
...
#include "define_histogram_eff.h"
#include "branch.h"
...
#include "scaler_analysis.h"
	...
	if(runs[r]<6000){
	  shms_etottrack_cut = pcaletottrack < pcaletotmax_peter;
	  hms_etottrack_cut  = hcaletottrack >  hcaletotmin_peter;
	}
    ...
    Double_t ctime_acc_ent        = hctimeacc->Integral();
    Double_t ctime_acc_1peak_ent  = hist_ctime_acc_1peak->Integral();
    Double_t ctime_coin_ent       = hctimect->Integral();
    Double_t ctime_clean_ent      = hist_ctime_clean->Integral();
    // cout<<"============================================================================"<<endl;
    ...
    Double_t ent_hcaldid_coin =  hist_hcal_did_coin->Integral();
    Double_t ent_hcaldid_acc =  hist_hcal_did_acc->Integral();
    Double_t ent_hcaldid_acc_1peak =  hist_hcal_did_acc_1peak->Integral();
    Double_t ent_hcaldid_clean =  hist_hcal_did_clean->Integral();


    Double_t ent_hcalshould_coin =  hist_hcal_should_coin->Integral();
    Double_t ent_hcalshould_acc =  hist_hcal_should_acc->Integral();
    Double_t ent_hcalshould_acc_1peak =  hist_hcal_should_acc_1peak->Integral();
    Double_t ent_hcalshould_clean =  hist_hcal_should_clean->Integral();

    //Find Bin
    // cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<endl;
    Double_t int_hcaldid_clean =  hist_hcal_did_clean->Integral(hist_hcal_did_clean->FindBin(hcaletotmin),hist_hcal_did_clean->FindBin(2));
    Double_t int_hcalshould_clean =  hist_hcal_should_clean->Integral(hist_hcal_should_clean->FindBin(0.),hist_hcal_should_clean->FindBin(2));
    ...
    Double_t int_pcaldid_clean =  hist_pcal_did_clean->Integral(hist_pcal_did_clean->FindBin(0),hist_pcal_did_clean->FindBin(pcaletotmax));
    Double_t int_pcalshould_clean =  hist_pcal_should_clean->Integral(hist_pcal_should_clean->FindBin(0),hist_pcal_should_clean->FindBin(2));
    ...
    Double_t ent_pcaldid_coin =  hist_pcal_did_coin->Integral();
    Double_t ent_pcaldid_acc =  hist_pcal_did_acc->Integral();
    Double_t ent_pcaldid_acc_1peak =  hist_pcal_did_acc_1peak->Integral();
    Double_t ent_pcaldid_clean =  hist_pcal_did_clean->Integral();

    Double_t ent_pcalshould_coin =  hist_pcal_should_coin->Integral();
    Double_t ent_pcalshould_acc =  hist_pcal_should_acc->Integral();
    Double_t ent_pcalshould_acc_1peak =  hist_pcal_should_acc_1peak->Integral();
    Double_t ent_pcalshould_clean =  hist_pcal_should_clean->Integral();

    Double_t hcal_eff = ent_hcaldid_clean/ent_hcalshould_clean;
    Double_t pcal_eff = ent_pcaldid_clean/ent_pcalshould_clean;
    Double_t hcal_eff_err = (1/ent_hcalshould_clean) * sqrt(ent_hcaldid_clean *( 1- hcal_eff ));
    Double_t pcal_eff_err = (1/ent_pcalshould_clean) * sqrt(ent_pcaldid_clean *( 1- pcal_eff ));
    ...
    txtfile2<<setprecision(5)<<runs[r]<<"\t"<<ent_hcaldid_coin<<"\t"<<ent_hcaldid_acc_1peak<<"\t"<<ent_hcaldid_clean<<"\t"<<ent_hcalshould_coin<<"\t"<<ent_hcalshould_acc_1peak<<"\t"<<ent_hcalshould_clean<<"\t"<<hcal_eff<<"\t"<<hcal_eff_err<<endl;
    txtfile3<<setprecision(5)<<runs[r]<<"\t"<<ent_pcaldid_coin<<"\t"<<ent_pcaldid_acc_1peak<<"\t"<<ent_pcaldid_clean<<"\t"<<ent_pcalshould_coin<<"\t"<<ent_pcalshould_acc_1peak<<"\t"<<ent_pcalshould_clean<<"\t"<<pcal_eff<<"\t"<<pcal_eff_err<<endl;

    cout<<setprecision(5)<<runs[r]<<"\t"<<ent_pcaldid_coin<<"\t"<<ent_pcaldid_acc_1peak<<"\t"<<ent_pcaldid_clean<<"\t"<<ent_pcalshould_coin<<"\t"<<ent_pcalshould_acc_1peak<<"\t"<<ent_pcalshould_clean<<"\t"<<pcal_eff<<"\t"<<pcal_eff_err<<endl;
    ...
    hist_hcal_did_clean->Draw("hist ");
    hist_hcal_did_clean->SetLineWidth(2);
    hist_hcal_did_clean->SetLineColor(kGreen);
    hist_hcal_did_coin->Draw("hist sames");
    hist_hcal_did_coin->SetLineWidth(2);
    hist_hcal_did_coin->SetLineColor(kBlue);
    hist_hcal_did_acc_1peak->Draw("hist sames");
    hist_hcal_did_acc_1peak->SetLineWidth(2);
    hist_hcal_did_acc_1peak->SetLineColor(kRed);
    hist_hcal_did_clean->SetTitle(Form("hist_hcal_did_clean %d",runs[r]));

    c1->cd(2);gPad->SetLogy();
    hist_hcal_should_clean->Draw("hist ");
    hist_hcal_should_clean->SetLineWidth(2);
    hist_hcal_should_clean->SetLineColor(kGreen);
    hist_hcal_should_coin->Draw("hist sames");
    hist_hcal_should_coin->SetLineWidth(2);
    hist_hcal_should_coin->SetLineColor(kBlue);
    hist_hcal_should_acc_1peak->Draw("hist sames");
    hist_hcal_should_acc_1peak->SetLineWidth(2);
    hist_hcal_should_acc_1peak->SetLineColor(kRed);


    c1->cd(3);gPad->SetGrid();
    hctime_pid->Draw("hist ");
    hctime_pid->SetLineWidth(2);
    hctime_pid->SetLineColor(kGreen);
    hctime->Draw("hist sames");
    hctime->SetLineWidth(2);
    hctime->SetLineColor(kBlue);
    hctimeacc->Draw("hist sames");
    hctimeacc->SetLineWidth(2);
    hctimeacc->SetLineColor(kRed);

    c1->cd(4);gPad->SetLogy();
    hist_pcal_did_clean->Draw("hist ");
    hist_pcal_did_clean->SetLineWidth(2);
    hist_pcal_did_clean->SetLineColor(kGreen);
    hist_pcal_did_coin->Draw("hist sames");
    hist_pcal_did_coin->SetLineWidth(2);
    hist_pcal_did_coin->SetLineColor(kBlue);
    hist_pcal_did_acc_1peak->Draw("hist sames");
    hist_pcal_did_acc_1peak->SetLineWidth(2);
    hist_pcal_did_acc_1peak->SetLineColor(kRed);

    c1->cd(5);gPad->SetLogy();
    hist_pcal_should_clean->Draw("hist ");
    hist_pcal_should_clean->SetLineWidth(2);
    hist_pcal_should_clean->SetLineColor(kGreen);
    hist_pcal_should_coin->Draw("hist sames");
    hist_pcal_should_coin->SetLineWidth(2);
    hist_pcal_should_coin->SetLineColor(kBlue);
    hist_pcal_should_acc_1peak->Draw("hist sames");
    hist_pcal_should_acc_1peak->SetLineWidth(2);
    hist_pcal_should_acc_1peak->SetLineColor(kRed);

    c1->cd(6);gPad->SetLogz();
    ctime_vs_rf->Draw("colz");

    c1->SaveAs(Form("pdfcaleff_sept30/cal_effplots_%d.pdf",runs[r]));
  }//runs
