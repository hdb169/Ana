...
void electrons_piminus_hgcer_eff(){//pos = +;neg =neg, posneg = +/- both
  ...
  // tt->Add("/home/hdbhatt/ROOTfiles/nov12_hgceff/all_neg_H2D2Al_rf0p51p5.root");
   //  tt->Add("/home/hdbhatt/ROOTfiles/oct28_hgceff/all_neg_H2D2Al_rf0p51p5.root");
 //tt->Add(Form("/home/hdbhatt/ROOTfiles/oct28_hgceff/all_%s_H2D2Al.root",polarity));

   tt->Add("/home/hdbhatt/ROOTfiles/hgcerEff_momGT2p7/piminus_gt2p7_H2D2Al.root");
 ofstream txtfile("txtfile/efficiency_neg_electrons.txt");
  txtfile<<setprecision(5)<<"binNum"<<"\t"<<"mom-"<<"\t"<<"eff+"<<"\t"<<"Efferr+"<<endl;


  //cout<<"data entries = "<<nentriesD<<"  "<<endl;
  double bin_mom = 7;
  double bin_mom_lo = 2.8;
  double bin_mom_hi = 4.8;
  double mom_lo = 2.8;// 3.0;
  double mom_hi = 4.8;//4.5;
  double pcal_pion = 0.7;
  double pcal_electron = 0.8;
  double rf_lo = 0.5;
  double rf_hi_pip = 1.5;
  double rf_hi_pim = 1.5;
  double rf_hi_electron = 1.5;
  ...
  TH1F *pmom = new TH1F("pmom","Momentum (-VE 0.5<RF<1.5: electron);Momentum (GeV);Entries", 100, bin_mom_lo, bin_mom_hi);
  TH1F *hctime_acc_pre = new TH1F("hctime_acc_pre","Ctime (-VE 0.5<RF<1.5: electron);Ctime (ns);Entries", 160,20,80);
  TH1F *hctime_acc = new TH1F("hctime_acc","Ctime (-VE 0.5<RF<1.5: electron);Ctime (ns);Entries", 160,20,80);
  TH1F *hctime_coin = new TH1F("hctime_coin","Ctime (-VE 0.5<RF<1.5: electron);Ctime (ns);Entries ", 160,20,80);
  ...
  TProfile2D * xy_vs_npe_grcut_with_hole = new TProfile2D("xy_vs_npe_grcut_with_hole","-VE 0.5<RF<1.5: electron: X vs Y  vs NPE (hole present, No Cer Cut);P.hgcer.yAtCer;P.hgcer.xAtCer;NPESUM", 300, -50, 50, 300, -50, 50, 0, 35);
  ...
  TH1F * htotal_mom_before_remove_region_acc  = new TH1F( "htotal_mom_before_remove_region_acc","Total before hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * hpass_mom_before_remove_region_acc  = new TH1F( "hpass_mom_before_remove_region_acc","Passed before hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * htotal_mom_after_remove_region_acc  = new TH1F( "htotal_mom_after_remove_region_acc","Total after hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * hpass_mom_after_remove_region_acc  = new TH1F( "hpass_mom_after_remove_region_acc","Passed after hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 


  TH1F * htotal_mom_before_remove_region_coin  = new TH1F( "htotal_mom_before_remove_region_coin","Total before hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * hpass_mom_before_remove_region_coin  = new TH1F( "hpass_mom_before_remove_region_coin","Passed before hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * htotal_mom_after_remove_region_coin  = new TH1F( "htotal_mom_after_remove_region_coin","Total after hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * hpass_mom_after_remove_region_coin  = new TH1F( "hpass_mom_after_remove_region_coin","Passed after hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 

  TH1F * htotal_mom_before_remove_region_clean  = new TH1F( "htotal_mom_before_remove_region_clean","Total before hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * hpass_mom_before_remove_region_clean  = new TH1F( "hpass_mom_before_remove_region_clean","Passed before hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * htotal_mom_after_remove_region_clean  = new TH1F( "htotal_mom_after_remove_region_clean","Total after hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  TH1F * hpass_mom_after_remove_region_clean  = new TH1F( "hpass_mom_after_remove_region_clean","Passed after hole removed (Mom); Momuntum (-VE 0.5<RF<1.5: electron); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 

  TH1F *hEff = new TH1F("hEff","Hgcer Efficiency;Momentum (-VE 0.5<RF<1.5: electron) GeV;Efficiency", bin_mom, bin_mom_lo, bin_mom_hi);
  ...
  for (int kk=0; kk<nentriesD;  kk++){
    ...
    //ctime_peak_finding = pcaletottracknorm < pcal_pion  && mom >= mom_lo &&mom   <=  mom_hi  && remove_region_acc;//pos
    ctime_peak_finding = pcaletottracknorm > pcal_electron  && mom >= mom_lo &&mom   <=  mom_hi  && remove_region_acc;//neg
  ...
  remove_region = central_hole && right_or_left_of_hole;//ok
    ...
    data_cut =  pcaletottracknorm > pcal_electron    &&  mom >= mom_lo  &&mom <= mom_hi && rftime < rf_hi_electron ; //electron selection by cal > 0.85 in shms pi-
  ...
  hpass_mom_hole_acc_1peak->SetTitle("Passed before hole removed (Mom): 4 Acc Plots (-VE 0.5<RF<1.5: electron)");
  c->cd(2);
 ...
 TGraphErrors* gr = new TGraphErrors();
  for (int i =0;i<hpass_mom_after_remove_region_clean->GetXaxis()->GetNbins();i++){
    ...
    txtfile<<setprecision(5)<<i<<"\t"<<x<<"\t"<<eff<<"\t"<<err<<endl;
  ...
  c1->SaveAs("pdf/base_plots_electron.pdf");
  c2->SaveAs("pdf/eff_plots_electron.pdf");
  c3->SaveAs("pdf/finaleff_plots_electron.pdf");
  c->SaveAs("pdf/accidental_only_electron.pdf");

  //effiecincy electrons
  TCanvas *c5 = new TCanvas("c5","c5",1000,800);
  c5->Divide(1,1);gPad->SetGrid();
  c5->cd(1);
  gr->Draw("AP");//efficiency
  ...
  gr->Fit("pol0");
    gPad->Update();
  // pEff_mom_after_remove_region->Draw("same");  
  c5->SaveAs("pdf_bin/eff_binerr_piminus_electron.gif");
  c5->SaveAs("pdf_bin/eff_binerr_piminus_electron.pdf");
