...
void piplus_hgcer_eff_BinerrHighRF(){//pos = +;neg =neg, posneg = +/- both
  ...
  gStyle->SetErrorX(0);
  // gStyle->SetOptStat(0);
  ofstream txtfile("txtfile/efficiency_pos.txt");
  txtfile<<setprecision(5)<<"binNum"<<"\t"<<"mom+"<<"\t"<<"eff+"<<"\t"<<"Efferr+"<<endl;
 
  TChain *tt = new TChain("T");
  
  tt->Add("/home/hdbhatt/ROOTfiles/hgcerEff_momGT2p7/piplus_gt2p7_H2D2Al.root");
  ...
  TH1F * heff  = new TH1F( "heff","Passed over total after hole removed (Mom); Momuntum (+VE, 0.5<RF<1.5); Entries", bin_mom, bin_mom_lo, bin_mom_hi); 
  ...
  heff->Draw();//empty
  heff->GetYaxis()->SetRangeUser(0.9,1);
  pEff_mom_after_remove_region->Draw("same");
  pEff_mom_after_remove_region->SetMarkerStyle(20);
  pEff_mom_after_remove_region->SetLineWidth(2);
  
  c1->SaveAs("pdf/base_plots_pos_binerr_highRF.pdf");
  c2->SaveAs("pdf/eff_plots_pos_binerr_highRF.pdf");
  c3->SaveAs("pdf/finaleff_pos_binerr_highRF.pdf");
  c->SaveAs("pdf/accidental_only_pos_binerr_highRF.pdf");
  ...
  // pEff_mom_after_remove_region->Draw("same");  
  c5->SaveAs("pdf_bin/eff_binerr_piplus.gif");
  c5->SaveAs("pdf_bin/eff_binerr_piplus_highRF.pdf");
  /*

  //example yield pi+ these are not true pos and neg, but just an example only
  ...
  c6->SaveAs("pdf_bin/yield_binerr1_highRF.pdf");
  */
}//void
