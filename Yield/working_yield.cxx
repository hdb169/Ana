//this is working version as of Jan 28, Friday, 022.
//this does take runs in similar rungroup for D2, does the yield with correct eff, 
//do the same for dummy and takes the dummy subtraction and finally does data and simc yield 
//comparison and makes a plot for each setting..
//run  by scriptname.cxx
...
std::vector<double> Get_weighted_averages(std::vector<int> RunNumbers,int bins){
  ...
  json j_info;
  {
    std::ifstream ifs("db2/runs_info_6073_trash.json");
    ifs>>j_info;
  }
  ...
  for(int i = 1;i<=bins;++i){
    ...
    for(auto it = RunNumbers.begin();it!=RunNumbers.end();++it){
      ...
      TFile *root_file =new TFile(("/home/hdbhatt/yield/yield_root/count_histo_"+std::to_string(RunNumber)+".root").c_str());//counts hists
...
std::vector<double> Get_weighted_errs(std::vector<int> RunNumbers,int bins){
  ...
  for(int i = 1;i<=bins;++i){
    ...
    for(auto it = RunNumbers.begin();it!=RunNumbers.end();++it){
      ...
      //TFile *root_file = new TFile(("results/yield/kinematics_yield_"+std::to_string(RunNumber)+".root").c_str());
      //TFile *root_file =new TFile(("/home/hdbhatt/yield/yield_root/count_histo_"+std::to_string(RunNumber)+".root").c_str());
      //TFile *root_file =new TFile(("/home/hdbhatt/yield/yield_root/count_histo_"+std::to_string(RunNumber)+".root").c_str());
      //  TFile *root_file =new TFile(("/home/hdbhatt/yield/yield_root/count_histo_"+std::to_string(RunNumber)+".root").c_str());
      TFile *root_file =new TFile(("/home/hdbhatt/yield/yield_root/count_histo_"+std::to_string(RunNumber)+".root").c_str());
    ...
    for(json::iterator it  = j_Q2z.begin();it!=j_Q2z.end();++it){
      ...
      if(xbj !=0 && Q2!=0){  
        for(json::iterator it = j_z.begin();it!=j_z.end();++it){
          ...
          TFile *rootfile_neg_sim = new TFile(("/home/hdbhatt/yieldsimc2022/results/yield/kinematics_yield_"+std::to_string(RunGroup)+"_simc.root").c_str());
          ...
          TFile *rootfile_pos_sim = new TFile(("/home/hdbhatt/yieldsimc2022/results/yield/kinematics_yield_"+std::to_string(RunGroup)+"_simc.root").c_str());
	  ...
	  /*       
   std::cout<<"Working...ok..."<<std::endl;


	  TCanvas *d2 = new TCanvas("d2","d2",1000,800);
	  d2->Divide(2,2);
	  d2->cd(1);
	  h_z_pos_sim->Draw();

	  d2->cd(2);
	  h_z_pos_sim_incrad->Draw();
	  d2->cd(3);

	  d2->cd(4);

	  d2->SaveAs("testsim.pdf");
*/
          ...
          if(z!=0){
            ...
            // std::cout<<"check neg first run "<<neg_D2_runs[1]<<std::endl;
            // std::cout<<"check neg first run "<<neg_D2_runs[2]<<std::endl;
            // std::cout<<"check neg first run "<<neg_D2_runs[3]<<std::endl;
            // std::cout<<"check neg first run "<<neg_D2_runs[4]<<std::endl;
            // std::cout<<"check neg first run "<<neg_D2_runs[5]<<std::endl;

	    std::cout<<"Working...ok....ok..."<<std::endl;
	    std::cout<<"bins "<<bins<<std::endl;

	    neg_D2_yields = Get_weighted_averages(neg_D2_runs,bins);
	    std::cout<<"Working...ok....ok...ok..."<<std::endl;
	    std::cout<<"bins "<<bins<<std::endl;
	    // std::cout<<"efficiency "<<efficiency<<std::endl;


            std::cout<<"neg_yields :"<<neg_D2_yields[1]<<std::endl;
            neg_D2_errs = Get_weighted_errs(neg_D2_runs,bins);
            pos_D2_yields = Get_weighted_averages(pos_D2_runs,bins);
            std::cout<<"pos_yields :"<< pos_D2_yields[40]<<std::endl;
	    ...
	    pos_D2_errs = Get_weighted_errs(pos_D2_runs,bins);
            ...
            std::cout<<neg_Dummy_yields[40]<<std::endl;
            std::cout<<pos_Dummy_yields[40]<<std::endl;
            x_axis = Get_x_axis(neg_D2_runs,bins);
            std::cout<<x_axis[1]<<std::endl;
          }//if z not 0
	    std::cout<<"Working...ok....ok...ok...11"<<std::endl;
	  ...
	  TH1* rp_radia_corr_neg = (TH1*)h_z_neg_incnorad->Clone("rp_radia_corr");
          ...
          std::string c_radia_pos_name = "/home/hdbhatt/yield/yieldJan_2022/results/yield/statistics_corr/radia_corr_neg"+q2xz_str_filename+"_pos.pdf";
          c_radia_pos->SaveAs(c_radia_pos_name.c_str());
          TCanvas* c_radia_neg = new TCanvas();
          rp_radia_corr_neg->Draw();
          std::string c_radia_neg_name = "/home/hdbhatt/yield/yieldJan_2022/results/yield/statistics_corr/radia_corr_neg"+q2xz_str_filename+"_neg.pdf";
          c_radia_neg->SaveAs(c_radia_neg_name.c_str());
	    std::cout<<"Working...ok....ok...ok...11.....11"<<std::endl;
	  ...
	  */
	    ...
	    // cout<<"test err + = "<< error<<" " <<error_dummy<<" "<<error_corr<<endl;
            if(y!=0){
	  ...
	  /*	  std::cout<<"Working...ok....ok...ok...11.....11..ittt..11"<<std::endl;

	  TCanvas *d1 = new TCanvas("d1","d1",1000,800);
	  d1->Divide(2,2);
	  d1->cd(1);
	  h_z_neg_sim_incrad->Draw();

	  d1->cd(2);
	  g_z_pos_yield_corr->Draw();
	  d1->cd(3);
	  g_z_pos_Dummy_yield->Draw();
	  d1->cd(4);
...
*/
	  ...
	  std::cout<<"check corr"<<std::endl;
          ...
          mg_z_yield_pos->Add(g_z_pos_sim,"L"); 
          ...
          mg_z_yield_pos->GetXaxis()->SetTitle("h_zhad_clean_mom_counts");
	  std::cout<<"check setting "<<std::endl;
          TCanvas* c_yield_pos = new TCanvas();
          mg_z_yield_pos->Draw("");//A
          c_yield_pos->BuildLegend(0.75,0.75,1,1);
	    std::cout<<"Hello Look"<<std::endl;

          std::string c_yield_pos_name = "/home/hdbhatt/yield/yieldJan_2022/results/yield/statistics_corr/yield_"+q2xz_str_filename+"_pos.pdf";
	    ...
	    //cout<<"test err - = "<< error<<" " <<error_dummy<<" "<<error_corr<<endl;
          ...
          mg_z_yield_neg->GetXaxis()->SetTitle("h_zhad_clean_mom_counts");
          mg_z_yield_neg->Draw("");//AL
          c_yield_neg->BuildLegend(0.75,0.75,1,1);
          std::string c_yield_neg_name = "/home/hdbhatt/yield/yieldJan_2022/results/yield/statistics_corr/yield_"+q2xz_str_filename+"_neg.pdf";
          ...
          g_yield_ratio->GetXaxis()->SetTitle("h_zhad_clean_mom_counts");
...
*/
  ...
  //mg_all->GetXaxis()->SetTitle("h_zhad_clean_mom_counts");
