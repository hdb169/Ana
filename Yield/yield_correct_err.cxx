//This script does accurate calculation of data yield, and does dummy subtraction, does simc yield add for 3 simc process and finally overlay each other
//not working but the script concept is clear...
...
std::vector<double> Get_weighted_averages(std::vector<int> RunNumbers,int bins){
  ...
  json j_info;
  {
    std::ifstream ifs("db2/runs_info_6073.json");
    ifs>>j_info;
  }
...
int yield_correct_err(){
  json j_Q2x;
  {
    std::ifstream runs("db2/kin_group_xQ2z_6073.json");
    runs>>j_Q2x;
  }
  json j_info;
  {
